...
function osztoMove(e, indx) {
    ...
    const oindx = _.last(jcsblokkok.filter(y => y < osztoelem)) || 1;
    if (indx == osztoelem)
        if ($(e).hasClass('move'))
            $(e).removeClass('move');
        else
...
function formAbontas(b) {
    ...
    var ossz = 0;
    if (b.length == 0) {
        txt += "<span  onclick='zetajcsHl(this," + szamlalo + ");'>1</span>&middot;";
        dtxt += "&middot;<span class='bvec' data-ab='" + szamlalo + "'>1</span>";
    } else
        for (let w of b) {
            var k = w[1];
            var v = w[0]
...
function kawashima() {
    ...
    if (a_sor != undefined && b_sor != undefined) {
        ...
        if (m == 1)
            ...
            txt = "<span style='display:inline-block;margin-bottom:5px;background-color:#ffccd4;border:1px solid #ff7b7b;padding:2px 4px;'><span class='block' style='transform:scale(1.5);margin-top:10px;margin-left: 1em;'>∑</span><sub style='vertical-align:-1.6em;margin-left:-2.2em;font-size:70%;'>0&lt;k&lt;" + m + "</sub><span style='font-size:110%;font-weight:700;margin-right:2px;'>&zeta;</span><span style='display:inline-block;transform-origin:center;transform:scale(1.1, 1.2);'>[</span><i>u</i>&sigma;((" + a0 + "))&nbsp;&circledast;&nbsp;(1)<sup>k</sup><span style='display:inline-block;transform-origin:center;transform:scale(1.1, 1.2);'>]</span>&nbsp;&middot;&nbsp;<span style='font-size:110%;font-weight:700;margin-right:2px;'>&zeta;</span><span style='display:inline-block;transform-origin:center;transform:scale(1.1, 1.2);'>[</span><i>u</i>&sigma;((" + b0 + "))&nbsp;&circledast;&nbsp;(1)<sup>" + m + " - k</sup><span style='display:inline-block;transform-origin:center;transform:scale(1.1, 1.2);'>]</span></span> = <br/>= " + txt;
            txt += "<hr/>";
...
function shuffleD() {
    ...
    const n1 = document.getElementById("xfok").value * 1;
    const n2 = document.getElementById("yfok").value * 1;
    var cw1 = document.getElementById("w1coeff").value;
    var cw2 = document.getElementById("w2coeff").value;
    if (/\//.test(cw1)) {
        var f1 = cw1.split("/")
        cw1 = Fraction(f1[0].trim() * 1, f1[1].trim() * 1)
    } else
        cw1 = Fraction(cw1 * 1);
    if (/\//.test(cw2)) {
        var f2 = cw2.split("/")
        cw2 = Fraction(f2[0].trim() * 1, f2[1].trim() * 1)
    } else
        cw2 = Fraction(cw2 * 1);
    var coeff = cw1.mul(cw2);
    var sh = polyShuffle(derivHn(w1, n1), derivHn(w2, n2));
    var fakt = 1;
    if (document.getElementById("derxfakt").checked)
        fakt *= factorial(n1);
    if (document.getElementById("derxfakte").checked)
        fakt *= Math.pow(-1, n1) * factorial(n1);
    if (document.getElementById("deryfakt").checked)
        fakt *= factorial(n2);
    if (document.getElementById("deryfakte").checked)
...
function html2maple() {
    const elem = document.getElementById("shouth");
    const txt = elem.innerText.replace(/[·|]/g, "");
    elem.innerHTML = txt
};
...
function setxyFakt(e) {
    const id = e.id;
    if (id == "derxfakt")
        document.getElementById("derxfakte").checked = false;
    else if (id == "derxfakte")
        document.getElementById("derxfakt").checked = false;
    else if (id == "deryfakt")
        document.getElementById("deryfakte").checked = false;
    else if (id == "deryfakte")
        document.getElementById("deryfakt").checked = false;
};
